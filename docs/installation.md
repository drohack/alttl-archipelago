...
because they have to agree about the item table. The mod checks this when it
connects and refuses a seed built by a different version, saying which two it
found - so a mismatch is now a clear message at the start rather than items
that do nothing halfway through a run.

A seed generated by an apworld older than this check cannot say which version
made it. The mod warns and plays it anyway, since refusing would strand runs
