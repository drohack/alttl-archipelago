...
/// </summary>
[BepInPlugin(Guid, "A Little To The Left Archipelago", "0.3.3")]
public sealed class Plugin : BasePlugin
{
    internal const string Guid = "droha.alttl.archipelago";

    /// <summary>
    /// This mod's version, "major.minor.build", read from the assembly.
    ///
    /// READ, NOT DECLARED, on purpose. The version already lives in three
    /// files that tools/check-version.py binds together - the csproj, the
    /// BepInPlugin attribute above, and the apworld manifest - and a fourth
    /// hand-written copy here would be a fourth thing to forget. The csproj's
    /// <Version> is what stamps the assembly, so this is the same number by
    /// construction.
    ///
    /// Used to refuse a seed generated by a different apworld: location ids
    /// move between releases, so a mismatched pair sends the wrong checks.
    /// </summary>
